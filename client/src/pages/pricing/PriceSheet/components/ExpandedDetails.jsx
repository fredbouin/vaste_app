...
const ExpandedDetails = ({ item, isComponent, settings, prices }) => {
  ...
  // Check if there's a manual price
  const hasManualPrice = item.manualPrice !== undefined && item.manualPrice !== null;

  // Calculate margin difference if manual price is set
  const getMarginInfo = () => {
    if (!hasManualPrice) return null;
    
    const manualPrice = Number(item.manualPrice);
    const difference = manualPrice - prices.msrp;
    const percentDiff = (difference / prices.msrp) * 100;
    const effectiveMargin = (1 - (computedTotalCost / manualPrice)) * 100;
  ...
  const marginInfo = hasManualPrice ? getMarginInfo() : null;
